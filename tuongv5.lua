...
-- Force Push Barrier v2.7 (SERVER-SIDE PUSH)
-- By Em 💕
--========================================
...
local Config = {
    ...
    SoundEnabled        = true,
    Active              = false,
    UseNetworkOwner     = true,   -- Cướp quyền điều khiển (mạnh nhất)
    UseAnchoredTrick    = true,   -- Neo cứng tạm thời
    UseCFrameForce      = true,   -- Cưỡng ép CFrame
}
...
local PushConn, PulseConn, RotateConn, PositionConn, WallConn
local EnemyCount = 0
local wallParts = {}
local networkOwned = {} -- Lưu những kẻ địch đã bị cướp network owner
...
--========================================
-- CƯỠNG ÉP ĐẨY KẺ ĐỊCH RA (CỰC MẠNH)
--========================================
local function forcePushEnemy(otherRoot, otherHum)
    ...
    --========================================
    -- BƯỚC 1: CƯỚP NETWORK OWNER (mạnh nhất)
    --========================================
    if Config.UseNetworkOwner then
        pcall(function()
            local otherPlayer = Players:GetPlayerFromCharacter(otherRoot.Parent)
            if otherPlayer and otherPlayer ~= Player then
                -- Cướp quyền điều khiển vị trí
                otherRoot:SetNetworkOwner(Player)
                networkOwned[otherPlayer.UserId] = true
            end
        end)
    end

    --========================================
    -- BƯỚC 2: ANCHORED TRICK (neo cứng tạm thời)
    --========================================
    if Config.UseAnchoredTrick then
        pcall(function()
            local wasAnchored = otherRoot.Anchored
            otherRoot.Anchored = true
            otherRoot.CFrame = CFrame.new(Vector3.new(targetPos.X, otherRoot.Position.Y, targetPos.Z))
            task.wait() -- Chờ 1 frame
            otherRoot.Anchored = wasAnchored
        end)
    end

    --========================================
    -- BƯỚC 3: CƯỠNG ÉP CFRAME (mỗi frame)
    --========================================
    if Config.UseCFrameForce then
        pcall(function()
            local look = otherRoot.CFrame.LookVector
            otherRoot.CFrame = CFrame.new(Vector3.new(targetPos.X, otherRoot.Position.Y, targetPos.Z), Vector3.new(targetPos.X, otherRoot.Position.Y, targetPos.Z) + look)
        end)
    end

    --========================================
    -- BƯỚC 4: BODY POSITION + VELOCITY (đẩy liên tục)
    --========================================
    ...
    bv.Velocity = dir * 1000

    -- Hiệu ứng
    local otherPlayer = Players:GetPlayerFromCharacter(otherRoot.Parent)
    local key = otherPlayer and otherPlayer.UserId or tostring(otherRoot)
    local now = tick()
    if not _G._pushCooldown then _G._pushCooldown = {} end
    if _G._pushCooldown[key] and now - _G._pushCooldown[key] < 0.3 then return end
    _G._pushCooldown[key] = now
...
--========================================
-- XÓA LỰC ĐẨY + TRẢ NETWORK OWNER
--========================================
local function clearPushFromEnemies()
        ...
        if otherPlayer ~= Player then
            ...
            if otherChar then
                ...
                if otherRoot then
                    ...
                    -- Trả network owner về cho họ
                    pcall(function()
                        otherRoot:SetNetworkOwner(otherPlayer)
                    end)
                end
            end
        end
...
--========================================
-- TẠO TƯỜNG VÔ HÌNH (3 LỚP - 108 PARTS)
--========================================
local function createInvisibleWall()
    ...
    -- LỚP 3 (dày hơn, chặn chắc hơn)
    for i = 1, numParts do
        local angle = (math.pi * 2 / numParts) * i + (math.pi / (numParts * 2))
        local wall = Instance.new("Part")
        wall.Name = "Wall3_" .. i
        wall.Anchored = true
        wall.CanCollide = true
...
local function createMenu()
    ...
    info.Text = "🛡️ 3 lớp tường vô hình\n🎯 Cướp Network + CFrame Force\n⌨️ T = Bật/tắt | H = Ẩn"
...
pcall(function()
    StarterGui:SetCore("SendNotification", {
        Title = "💜 PURPLE FIELD v2.7",
        Text = "Cướp Network Owner + 3 lớp tường vô hình!",
        Duration = 5
    })
end)

print("═══════════════════════════════════════════")
print("💜 PURPLE MAGNETIC FIELD v2.7 - LOADED!")
print("🛡️ 3 lớp tường vô hình (108 parts)")
print("🎯 Cướp Network Owner + CFrame Force")
